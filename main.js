...
function TestWeightCost(i) {
  ...
  */
  ...
  for (let j=0; j<j2; j++) {
    for (let k=0; k<j3; k++) {
      tempmatrix.set(j,k,Number(temparray.get(j) * temparray2.get(k)))
    }
  }
  return tempmatrix
...
function TestBackprop() {
  ...
  for (let i=layers-2; i>-1; i--) {
    ...
    document.getElementById("layers").innerHTML = "weight" + JSON.stringify(testweights[i+1]) + JSON.stringify(nj.multiply(TestWeightCost(i+1),learnrate)) + nj.zeros([2]).subtract(nj.array([-0.5,-0.5]))
