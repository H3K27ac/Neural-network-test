...
function TestBackprop() {
  ...
  for (let i=layers-2; i>-1; i--) {
    ...
    document.getElementById("layers").innerHTML = "weight" + JSON.stringify(testweights[i+1]) + JSON.stringify(nj.multiply(TestWeightCost(i+1),learnrate)) + nj.zeros([3,2]).subtract(nj.ones([3,2]))
